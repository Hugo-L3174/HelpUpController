...
CoMQP::CoMQP() : considerConstrainedContacts_(true) {}

CoMQP::CoMQP(std::shared_ptr<ContactSet> contactSet) : considerConstrainedContacts_(true)
{
...
void CoMQP::updateProblem()
{
  ...
  else
  {
    ...
    // number of inequalities = number of forces (conervative and non conservative) *( number of friction sides  + 2 for
    // upper and lower limits)
    // + 6 for com position boundaries
    nrInEq = (numContacts + numContacts - numConstrainedContacts) * (numFrictionSides + 2) * numAccelerations
             + 6; // + comRegionPlanes_.size();

    // mc_rtc::log::info("[CoMQP] nrVar: {}, nrEq: {}, nrInEq: {}", nrVar, nrEq, nrInEq);
  }
...
bool CoMQP::solveNonConstrained(Eigen::Vector3d currentCoM)
{
  ...
  P.bottomRightCorner<3, 3>() = 100000 * Eigen::Matrix3d::Identity();
...
bool CoMQP::solveConstrained(Eigen::Vector3d currentCoM)
{
  ...
  // // number of variable = non-conservative case forces + conservative case forces + com position
  int nrVar =
      (3 * numContacts) * numAccelerations + (3 * (numContacts - numConstrainedContacts)) * numAccelerations + 3;
  // number of equalities = non consercative case + conservative case
  int nrEq = 6 * numAccelerations + 6 * numAccelerations;
  // number of inequalities = number of forces (conervative and non conservative) *( number of friction sides  + 2 for
  // upper and lower limits)
  // + 6 for com position boundaries
  int nrInEq = (numContacts + numContacts - numConstrainedContacts) * (numFrictionSides + 2) * numAccelerations
               + 6; // + comRegionPlanes_.size();xs
  ...
  std::vector<std::string> names = {"RightHandPad_0", "RightHandPad_1", "RightHandPad_2", "RightHandPad_3",
                                    "RightHandPad_4"};
  ...
  Eigen::MatrixXd P = Eigen::MatrixXd::Identity(nrVar, nrVar);
  P.block(numConservativeForcesVar, numConservativeForcesVar, 3 * numConstrainedContacts, 3 * numConstrainedContacts) =
      100 * Eigen::MatrixXd::Identity(3 * numConstrainedContacts, 3 * numConstrainedContacts);
  P.bottomRightCorner<3, 3>() = 100000 * Eigen::Matrix3d::Identity();
  // std::cout << "P: \n" << P << std::endl;
