...
bool RobotTakePoseObserved::run(mc_control::fsm::Controller & ctl_)
{
  ...
  auto updatedObjectiveRH = ctl.realRobot(RHtargetFrame("robot")).frame(RHtargetFrame("frame")).position();
  ...
  // cheating for now and get real pose to test
  auto updatedObjectiveLH = ctl.realRobot(LHtargetFrame("robot")).frame(LHtargetFrame("frame")).position();
  // update offset of objective (transform between control frame and observed frame)
