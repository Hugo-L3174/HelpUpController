...
void RobotTakePoseObserved::start(mc_control::fsm::Controller & ctl_)
{
  ...
  if (config_.has("RHandTrajectory"))
  {
    ...
    critRH_.configure(*RHandTrajectoryTask_, ctl.solver().dt(), config_("RHandTrajectory")("completion", mc_rtc::Configuration{}));
...
bool RobotTakePoseObserved::run(mc_control::fsm::Controller & ctl_)
{
  ...
  auto updatedObjectiveRH = RHobjectiveOffset_ * ctl.realRobot(RHtargetFrame("robot")).frame(RHtargetFrame("frame")).position();
  ...
  // cheating for now and get real pose to test
  auto updatedObjectiveLH = LHobjectiveOffset_ * ctl.realRobot(LHtargetFrame("robot")).frame(LHtargetFrame("frame")).position();
  // update offset of objective (transform between control frame and observed frame)
