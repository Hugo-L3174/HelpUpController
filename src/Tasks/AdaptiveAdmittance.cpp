...
#include "Tasks/AdaptiveAdmittance.h"
...
AdaptiveAdmittance::AdaptiveAdmittance(const std::string & surfaceName,
                                       const mc_rbdyn::Robots & robots,
                                       unsigned int robotIndex,
                                       double stiffness,
                                       double weight)
: AdaptiveAdmittance(robots.robot(robotIndex).frame(surfaceName), stiffness, weight)
{
}

AdaptiveAdmittance::AdaptiveAdmittance(const mc_rbdyn::RobotFrame & frame, double stiffness, double weight)
: TransformTask(frame, stiffness, weight)
{
  if(!frame.hasForceSensor())
  {
    mc_rtc::log::error_and_throw("[mc_tasks::AdaptiveAdmittance] Frame {} does not have a force sensor attached",
                                 frame.name());
  }
...
void AdaptiveAdmittance::update(mc_solver::QPSolver &)
{
...
void AdaptiveAdmittance::load(mc_solver::QPSolver & solver, const mc_rtc::Configuration & config)
{
...
void AdaptiveAdmittance::addToLogger(mc_rtc::Logger & logger)
{
...
void AdaptiveAdmittance::addToGUI(mc_rtc::gui::StateBuilder & gui)
{
  gui.addElement(
      {"Tasks", name_},
...
void AdaptiveAdmittance::addToSolver(mc_solver::QPSolver & solver)
{
  timestep_ = solver.dt();
...
static auto registered = mc_tasks::MetaTaskLoader::register_load_function(
    ...
    [](mc_solver::QPSolver & solver, const mc_rtc::Configuration & config)
    {
      auto frame = [&]() -> std::string
      {
        ...
        {
          mc_rtc::log::deprecated("AdaptiveAdmittanceLoader", "surface", "frame");
          return config("surface");
        }
        return config("frame");
      }();
      auto rIndex = robotIndexFromConfig(config, solver.robots(), "admittance");
      auto t = std::make_shared<mc_tasks::force::AdaptiveAdmittance>(solver.robots().robot(rIndex).frame(frame));
      t->reset();
