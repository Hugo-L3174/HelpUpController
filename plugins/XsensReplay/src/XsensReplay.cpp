...
void XsensReplay::init(mc_control::MCGlobalController & gc, const mc_rtc::Configuration & config)
{
  ...
  {
    log_ = ds.get<decltype(log_)>("XsensReplay::Log");
  }
  else
  {
    if(!config.has("log"))
    {
      mc_rtc::log::warning("[XsensReplay] No log specified in the plugin configuration and no log available in the "
                           "datastore at XsensReplay::Log");
    }
    else
    {
      init_log(config("log"), ds);
    }
  }
  ...
  if(config_str.size())
  {
    mc_rtc::log::info("[XsensReplay] Will {}", config_str);
  }
  else if(log_to_datastore_.empty())
  {
    mc_rtc::log::warning("[XsensReplay] Configured to do nothing?");
  }
...
void XsensReplay::init_log(const std::string & logPath, mc_rtc::DataStore & ds)
{
  ...
  if(!ds.has("XsensReplay::Log"))
  {
    ds.make<decltype(log_)>("XsensReplay::Log", log_);
  }
  if(log_->size() == 0)
  {
    mc_rtc::log::error_and_throw("[XsensReplay] Cannot replay an empty log");
  }
...
void XsensReplay::reset(mc_control::MCGlobalController & gc)
{
  ...
  {
    if(ds.has(ds_update.ds_entry))
    {
      ds.remove(ds_update.ds_entry);
    }
  }
  ...
  for(auto it = log_to_datastore_.begin(); it != log_to_datastore_.end();)
  {
    ...
    {
      mc_rtc::log::error("[XsensReplay] Requested to map {} to {} but {} is not in the log", log_entry, ds_entry,
                         log_entry);
  ...
  gc.controller().gui()->addElement(
      ...
      mc_rtc::gui::Input("Skip iter (slow down factor)", skipIter_));
  gc.controller().gui()->addElement(
...
void XsensReplay::after(mc_control::MCGlobalController & gc)
{
  ...
  {
    if(skipIter_ != 0 && iters_ != 0 && totalIter_ % skipIter_ == 0)
    {
      iters_++;
    }
    totalIter_++;
